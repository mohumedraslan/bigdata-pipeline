"""
simulator/src/parking_state_machine.py
───────────────────────────────────────
Models a truck performing a reversing-into-spot manoeuvre.
The state machine advances through realistic phases so that
the sensor readings look physically plausible rather than random.

Physics model (simplified):
  - Truck approaches from ~500 cm out
  - Aligns within ±20 cm of spot centre-line
  - Reverses slowly into the spot
  - Confirms all-round clearance before declaring PARKED
"""
...
# ── Thresholds (cm) ───────────────────────────────────────────────────────────
APPROACH_DONE   = 150   # switch from APPROACHING → ALIGNING when front < this
ALIGN_DONE      = 80    # switch to REVERSING when lateral error < this
REVERSE_DONE    = 25    # switch to CONFIRMING when rear < this
CONFIRM_DONE    = 20    # switch to PARKED when all sides stable
EMERGENCY_STOP  = 12    # hard-stop any phase
...
class TruckUnit:
    """
    Simulates one truck performing a single parking session.

    Attributes
    ----------
    unit_id   : e.g. "TRUCK_001"
    spot_id   : e.g. "SPOT_B4"
    session_id: UUID for the whole manoeuvre
    ...
    _front: float  = field(default=500.0, repr=False)  # cm
    ...
    _servo: int    = field(default=90,    repr=False)   # degrees (90 = straight)
    _speed: float  = field(default=0.0,  repr=False)   # m/s
    ...
    def step(self) -> Tuple[SensorReading, ParkingDecision]:
        """
        Advance physics one tick and return (SensorReading, ParkingDecision).
        """
