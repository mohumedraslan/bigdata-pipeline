...
──────────────────────
Entrypoint for the sensor simulator.

• Spins up N TruckUnit state machines (one per truck)
• Each tick: advances physics, emits SensorReading → parking.sensors.raw
                                       ParkingDecision → parking.decisions
• Resets each truck after it successfully parks (infinite loop)

Firebase note
─────────────
In production the sensor readings come from Firebase Realtime Database.
This simulator produces identical JSON payloads so swapping the source
requires only changing the ingest point – the rest of the pipeline is untouched.
"""
...
def main() -> None:
    ...
    while True:
        for unit in units:
            ...
            # ── Reset truck after successful park ──────────────────────────
            if unit.is_done():
